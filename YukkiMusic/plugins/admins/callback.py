...
from RadhaXMusic import YouTube, app
from RadhaXMusic.core.call import RadhaX
from RadhaXMusic.misc import SUDOERS, db
from RadhaXMusic.utils.database import (is_active_chat,
...
from RadhaXMusic.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from RadhaXMusic.utils.stream.autoclear import auto_clean
from RadhaXMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await RadhaX.pause_stream(chat_id)
        ...
        await RadhaX.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            _["admin_4"].format(mention)
        )
    elif command == "Stop" or command == "End":
        await CallbackQuery.answer()
        await RadhaX.stop_stream(chat_id)
        ...
        await RadhaX.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await RadhaX.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await RadhaX.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await RadhaX.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
